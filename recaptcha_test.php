...
if (filter_input(INPUT_SERVER, 'REQUEST_METHOD') == "POST") {
    ...
    $response = file_get_contents($url . "?secret=" . $secretKey . "&response=" . $captcha . "&remoteip=" . $ip);
    $responseKeys = json_decode($response, true);
    if (intval($responseKeys["success"]) !== 1) {
        $Gmessage = "Captcha server response: error!";
        $message = 'Captcha Validation failed.Please try again';
        unset($_SESSION['username']);
    } else {
        $Gmessage = "Captcha server response: success!";
        if (isset($name)) {
            if (empty($name)) {
        ...
        <div class="main">
            <div class="main-div">
                ...
                <form class="row" method="POST" action="recaptcha_test.php" name="myform">
                    ...
                    <br><br>
                    <center>
                        <div class="smallcaptcha g-recaptcha" data-sitekey="6LfTdSgUAAAAAEb_nxNUIizLdRwFOK5f1saZGVHJ"></div>
                    </center>
                    <br>
